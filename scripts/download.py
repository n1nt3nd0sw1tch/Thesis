"""Downloads the source corpora and records what was retrieved.

Where each one comes from is set in config/settings.yml under datasets; this
script holds only the fetching, the hashing and the documentation table. A file
already present is left alone, so a rerun costs nothing.
...
def write_documentation(summary, doc_path):
    lines = ['# Datasets', '',
             f'Generated by `scripts/download.py` on {date.today().isoformat()}.',
